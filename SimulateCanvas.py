...
class SimulateCanvas(FigureCanvas):
    ...
    def __init__(self, x, y, backup, T_max, T, old_depth, V_chute, release_ind, instru_name, x_instru, 
        ...
        # plot du profil + point pour intrument
        plt.plot(x, y, x_instru, y_instru, 'ro')
        plt.xlabel("Dx (m)")
        plt.ylabel("Depth(m)")
        ...
        for i in range(len(x_instru)):  # on rajoute le nom des instrument
            ...
            if (abs(y1-y_prec)/(y[0]-y[-1])) < 4./200:
                # si les instruments sont trop serre, il y a un petit offset pour eviter un chevauchement des nom
                y1 = y_prec-(4./200)*(y[0]-y[-1])
            y_prec = y1
            self.axes1.text(x1, y1, instru_name[i], fontsize=8)
            plt.text(x1, y1, instru_name[i], fontsize=8)

        plt.savefig(path+'//Report//Pictures//Fig1.png',
        ...
        plt.figure(figsize=(4, 8))
        # On sarrete aux largueurs le graphique nest pas pertinent pour le reste de la ligne
        plt.plot(backup[1:], y[1:release_ind], zero, y[1:release_ind])
        ...
        plt.tick_params(axis='both', which='minor', labelsize=7)
        plt.grid(True)
        plt.savefig(path+'//Report//Pictures//Fig2.png',
                    dpi=180, bbox_inches='tight', pad_inches=0.1)
        plt.close()
        #version axes pour subplot#
        ...
        plt.tick_params(axis='both', which='minor', labelsize=7)
        plt.grid(True)
        plt.savefig(path+'//Report//Pictures//Fig3.png',
                    dpi=180, bbox_inches='tight', pad_inches=0.1)
        plt.close()
        #version axes pour subplot#
        ...
        plt.tick_params(axis='both', which='minor', labelsize=7)
        plt.grid(True)
        plt.savefig(path+'//Report//Pictures//Fig4.png',
                    dpi=180, bbox_inches='tight', pad_inches=0.1)
        plt.close()
        #version axes pour subplot#
        ...
        fig.subplots_adjust(left=0.1,  right=0.95, bottom=0.12,
                            top=0.92,  wspace=0.4,  hspace=0.05)
