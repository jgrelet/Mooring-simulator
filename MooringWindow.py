...
class MooringWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        #Fonction de zoom#
        zoom_in_Action = QtWidgets.QAction(QtGui.QIcon(
            self.mypath+'\Pictures\Icons\zoom_in.png'), 'Zoom_in', self)
        zoom_out_Action = QtWidgets.QAction(QtGui.QIcon(
            self.mypath+'\Pictures\Icons\zoom_out.png'), 'Zoom_out', self)
        zoom_in_Action.triggered.connect(
            functools.partial(self.zoom_mooring, 1))
        zoom_out_Action.triggered.connect(
            functools.partial(self.zoom_mooring, 0))
        self.current_zoom = 0
        ...
        self.toolbar = QtWidgets.QToolBar()  # Toolbar en haut de la fenetre
        self.toolbar.addAction(zoom_in_Action)
        self.toolbar.addAction(zoom_out_Action)
        self.toolbar2 = QtWidgets.QToolBar()  # Toolbar en bas de la fenetre
        label_toolbar = QtWidgets.QLabel("<b>Add new element<\b>")
        label_toolbar2 = QtWidgets.QLabel("<b>Insert new element<\b>")
        ...
        self.moor_bigLayout = QtWidgets.QVBoxLayout()
        self.moor_bigLayout.addWidget(self.toolbar)
        self.moor_groupLayout = QtWidgets.QVBoxLayout()
        #Taille des boutons#
        self.size_pol = 10
        ...
        self.moor_scrollArea = QtWidgets.QScrollArea()
        self.moor_groupLayout.addWidget(self.moor_scrollArea)

        self.scrolledWidget = QtWidgets.QWidget()
        self.mylayout = QtWidgets.QGridLayout(self.scrolledWidget)
        #Bouton insertion premier element#
        self.button_insert_first = QtWidgets.QPushButton(
            "Insert First Element", self)
        self.button_insert_first.clicked.connect(self.insert_first_element)
        self.button_insert_first.setFixedSize(
            110*self.a*self.k_mult, 22*(self.size_pol/10))
    ...
    def dragEnterEvent(self, event):
        ...
        if event.mimeData().hasFormat('image/piece'):  # Bon format
            ...
            dataStream = QtCore.QDataStream(
                pieceData, QtCore.QIODevice.ReadOnly)
            pixmap = QtGui.QPixmap()
            location = QtCore.QPoint()
            # on recupere le numero de la feuille excel et numero de la ligne de l element que veut clamper l utilisateur ##
    ...
    def dropEvent(self, event):
        ...
        self.piece_position = self.get_piece_position()
        if event.mimeData().hasFormat('image/piece'):
            for i in range(len(self.piece_position)):
                # On fait la difference entre la position du curseur et la position de chaque image
                tab.append(
                    abs(self.piece_position[i]-(event.pos().y()+self.vbar.value())))
            # On prend limage la plus proche comme support de clampage
            ind = tab.index(min(tab))
            if self.ropes_length[ind] != 0:  # Si le cable nest pas defini en auto
                reply = QtWidgets.QMessageBox.question(self, 'Message',
                                                       ...
                                                       str(self.tab_object[self.liste_element[ind]
                                                                           [0]][self.liste_element[ind][1]].name),
                                                       QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)

                if reply == QtWidgets.QMessageBox.Yes:
                    self.liste_element[ind] = [self.liste_element[ind], [
                        self.piece_sheet, self.piece_ind]]  # On ajoute l element clampe dans liste_element
                    event.accept()
                    if self.tab_name_class[self.piece_sheet] == "Instruments":
                        self.clamp = 1
                        # On demande la profondeur de l instrument
                        self.ask_instru_depth(ind)
                        instru_depth = self.instru_depth_value
                        self.clamp = 0
                        if instru_depth.toFloat()[1] == True:
                            instru_depth = instru_depth.toFloat()[0]

                    # On ajoute la profondeur choisie dans instru_depth
                    self.instru_depth[ind] = [
                        self.instru_depth[ind], instru_depth]
                ...
                QtWidgets.QMessageBox.warning(
                    self, 'Message', "You can't clamp on an auto rope")
                event.ignore()
    ...
    def get_piece_position(self):
        ...
        table.remove(77)
        # On recupere la position des centres des images
        for i in range(len(self.liste_element)):
            pixmap = self.mylayout.itemAtPosition(i+1, 1).widget().pixmap()
    ...
    def ask_instru_depth(self, ind):
        ...
        label = QtWidgets.QLabel(
            'Enter the depth of the instrument (Auto for automatic):')
        auto = QtWidgets.QPushButton("Auto")
        ok = QtWidgets.QPushButton("Ok")
        auto.clicked.connect(functools.partial(
            self.instru_auto_clicked_on, ind))
    ...
    def instru_ok_clicked_on(self, ind):
        ...
        self.dialog.close()
        if self.clamp == 1:
            # Si on est dans le cas d un clampage on apelle ask_ratio
            self.ask_ratio(ind)
    ...
    def ask_ropes_length(self):
        ...
        self.le = QtWidgets.QLineEdit(self)
        self.dialog = QtWidgets.QDialog(self)
        label = QtWidgets.QLabel('Enter the length  (Auto for automatic):')
        auto = QtWidgets.QPushButton("Auto")
        ok = QtWidgets.QPushButton("Ok")
    ...
    def ask_ratio(self, ind):
        ...
        self.le = QtWidgets.QLineEdit(self)
        self.dialog = QtWidgets.QDialog(self)
        label = QtWidgets.QLabel('Enter the clamp ratio (value from 0 to 1)')
        auto = QtWidgets.QPushButton("Auto")
        ok = QtWidgets.QPushButton("Ok")
        auto.clicked.connect(functools.partial(
            self.auto_ratio_clicked_on, ind))
    ...
    def ok_ratio_clicked_on(self, ind):
        ...
        if float(self.le.text()) > 0 and float(self.le.text()) < 1:
            ...
            QtWidgets.QMessageBox.warning(
                self, 'Message', "Ratio value is between 0 and 1")
    ...
    def ask_anchor_weight(self):
        ...
        self.le = QtWidgets.QLineEdit(self)
        self.dialog = QtWidgets.QDialog(self)
        label = QtWidgets.QLabel('Enter the wet weight of the anchor (kg):')
        ok = QtWidgets.QPushButton("Ok")
    ...
    def update_grid(self, vbar_statut):
        ...
        self.mylayout = QtWidgets.QGridLayout(scrolledWidget)
        ...
        for i in range(len(self.liste_element)):  # On parcourt la liste des elements
            # En cas de cable dont la longueur est connu on la rend visible
            if self.ropes_length[i] != 0:
                off = 1
                line_edit_length = QtWidgets.QLineEdit(
                    str(self.ropes_length[i]))
            else:
                off = 0
            if i == (len(self.liste_element)-1) and self.anchor_weight_value != '0':
                off2 = 1

                # En cas de dernier element si le poids du lest est defini on rend visible celui ci
                line_edit_weight = QtWidgets.QLineEdit(
                    str(self.anchor_weight_value))
            else:
                off2 = 0
            # En cas de clampage on ajoute le bouton unclamp
            if type(self.liste_element[i][0]) != int:

                button_unclamp = QtWidgets.QPushButton("Unclamp Element", self)
                button_unclamp.setFixedSize(
                    100*self.a*self.k_mult, 22*(self.size_pol/10))
                button_unclamp.setFont(self.f)
                button_unclamp.clicked.connect(
                    functools.partial(self.unclamp_element, i))
            ...
            button_exit = QtWidgets.QPushButton("Delete Element", self)
            button_insert = QtWidgets.QPushButton("Insert Element", self)
            button_exit.setFixedSize(
                100*self.a*self.k_mult, 22*(self.size_pol/10))
            button_insert.setFixedSize(
                100*self.a*self.k_mult, 22*(self.size_pol/10))
            button_exit.setFont(self.f)
            button_insert.setFont(self.f)
            button_exit.clicked.connect(
                functools.partial(self.delete_element, i))
            button_insert.clicked.connect(
                functools.partial(self.insert_element, i))
            #On redimensionne l image selon le zoom#
            new_Pixmap = pixmap.scaledToWidth(
                pixmap.width()*self.k_mult, QtCore.Qt.SmoothTransformation)
            label = QtWidgets.QLabel()
        ...
        if vbar_statut == 0:  # En cas d ajout d element la barre vertical prend sa valeur max
            ...
            # En cas dinsertion suppression clampage elle garde sa valeur
            self.vbar.setValue(self.vbar_value)
    ...
    def create_mooring_image(self):
        ...
        for i in range(len(self.liste_element)):
            ...
            if type(self.liste_element[i][0]) != int:
                path = self.mypath + \
                    self.tab_object[self.liste_element[i][0][0]
                                    ][self.liste_element[i][0][1]].image_file
                a = 0.75*k * \
                    len(self.tab_object[self.liste_element[i]
                                        [0][0]][self.liste_element[i][0][1]].name)
            else:
                path = self.mypath + \
                    self.tab_object[self.liste_element[i][0]
                                    ][self.liste_element[i][1]].image_file
                a = 0.75*k * \
                    len(self.tab_object[self.liste_element[i]
                                        [0]][self.liste_element[i][1]].name)
            newImage = QtGui.QPixmap()
            if not newImage.load(path):
                QtWidgets.QMessageBox.warning(
                    self, "Open Image", "The image file could not be loaded.", QtWidgets.QMessageBox.Cancel)
                return
        ...
        for i in range(len(self.liste_element)):
            if type(self.liste_element[i][0]) == int:  # element non clampe
                ...
                newImage = QtGui.QPixmap()
                newImage.load(path)
                name = self.tab_object[self.liste_element[i]
                                       [0]][self.liste_element[i][1]].name
                # On cree l image resultante avec une constante en largeur et la hauteur de l image dorigine
                ImageResult2 = QtGui.QPixmap(
                    max(self.max_len)+max(self.max_width)+1*offset, newImage.height())
                ImageResult2.fill(QtCore.Qt.white)  # On la remplit de blanc
                painter = QtGui.QPainter(ImageResult2)
                painter.setFont(font)
                # On ecrit le nom de l element a droite et centre en hauteur
                painter.drawText(QtCore.QPoint(max(
                    self.max_width)+offset, newImage.height()-5-abs(newImage.height()-23)/2), name)
                painter.drawPixmap(((max(self.max_width)-newImage.width())/2) +
                                   offset-offset/4, 0, newImage)  # On dessine l image
                if self.instru_depth[i] != 0:
                    depth = str(self.instru_depth[i])
                    # Si la profondeur est connu celle ci est dessine a gauche
                    painter.drawText(QtCore.QPoint(
                        35, newImage.height()-5-abs(newImage.height()-23)/2), depth)

            else:
                #En cas de clampage#
                ImageResult2 = self.create_clamp_image(
                    self.liste_element[i])  # Creation de l image
                if type(self.instru_depth[i]) == list:
                    for p in range(2):
                        if self.instru_depth[i][p] != 0:
                            ...
                            # Si la profondeur est connu celle ci est dessine a gauche
                            painter.drawText(QtCore.QPoint(
                                35, ImageResult2.height()-5-abs(ImageResult2.height()-23)/2), depth)
    ...
    def create_clamp_image(self, element):
        ...
        if newImage1.height() < newImage2.height():  # Si le support est plus petit que l element clampe
            newImage2 = newImage2.scaledToHeight(newImage1.height(
            ), QtCore.Qt.SmoothTransformation)  # On redimensionne l element clampe
        ImageResult2 = QtGui.QPixmap(
            max(self.max_len)+max(self.max_width)+1*offset, newImage1.height())
        ImageResult2.fill(QtCore.Qt.white)
        painter = QtGui.QPainter(ImageResult2)
        painter.setFont(font)
        #Dessine les deux images#
        painter.drawPixmap(
            ((max(self.max_width)-newImage1.width())/2)+offset-offset/4, 0, newImage1)
        painter.drawPixmap(((max(self.max_width)-newImage1.width())/2)+offset-offset /
                           4-newImage2.width(), (newImage1.height()-newImage2.height())/2, newImage2)
        #Ajout du texte#
        painter.drawText(QtCore.QPoint(max(self.max_width)+offset,
                                       ImageResult2.height()-5-abs(ImageResult2.height()-3*23)/2), name1)
        painter.drawText(QtCore.QPoint(max(self.max_width)+offset,
                                       ImageResult2.height()-5-(abs(ImageResult2.height()-3*23)/2+23)), text)
        painter.drawText(QtCore.QPoint(max(self.max_width)+offset,
                                       ImageResult2.height()-5-(abs(ImageResult2.height()-3*23)/2+46)), name2)
    ...
    def delete_element(self, k):
        ...
        if self.insert != 1:
            ...
            QtWidgets.QMessageBox.information(
                self, 'Message', "Choose an element to insert")
...
class ObjectList(QtWidgets.QListWidget):
    ...
    def addObject(self, pixmap, sheet, nb):
        ...
        # On ajoute les informations dont on a besoin dans l item
        pieceItem.setData(QtCore.Qt.UserRole, [pixmap, sheet, nb])
...
class MooringWindow(QtWidgets.QWidget):
    ...
    def __init__(self, default_folder, screen_width, parent=None):
        ...
        self.mypath = self.mypath+'\Library'
        #Creation toolbar#
        refresh_library = QtWidgets.QAction(QtGui.QIcon(
            self.mypath+'\Pictures\Icons\Refresh.png'), 'Refresh_library', self)
        ...
        # Contient la toolbar et les objectlist
        self.frameLayout2 = QtWidgets.QVBoxLayout()
        # Contient le mooringWidget et le framelayout2
    ...
    def create_widget(self):
        ...
        self.get_library(
            self.default_folder)  # Chargement des donnees de Library.xls#
        ...
        for i in range(len(self.tab_object)):
            ...
            for j in range(len(self.tab_object[i])):
                # On ajoute chaque objet
                objectList.addObject(self.image[i][j], i, j)

            # On connecte le double clique avec la fonction add_new_element
            objectList.itemDoubleClicked.connect(
                functools.partial(self.add_new_element, i))
            ObjectList_subwind = QtWidgets.QMdiSubWindow()
        ...
        for i in range(len(self.tab_object)):
            ...
            for j in range(len(self.tab_object[i])):
                ...
                for a in text:
                    if a.isupper():
                        cap = cap+1
                # Cap designe le nombre de majuscule
                true_len = len(text)+cap*0.5
                if true_len > 16:  # Si la chaine est trop longue on la divise en deux
                    ...
                    if len(mytext) == 1:
                        ...
                        if len(mytext) == 1:
                            ...
                            if len(mytext) == 1:
                                mytext = [
                                    text[0:int(len(text)/2)], text[int(len(text)/2)+1:len(text)-1]]
                    if h > 114:  # On a besoin de 36 pixel pour ecrire les deux chaines de charactere l un sous l autre
                        if(w/h)*114 <= 150:
                            newImage = newImage.scaledToHeight(
                                114, QtCore.Qt.SmoothTransformation)
                            w, h = newImage.width(), newImage.height()
                        else:
                            newImage = newImage.scaledToWidth(
                                150, QtCore.Qt.SmoothTransformation)
                            w, h = newImage.width(), newImage.height()
                    painter.setFont(font)
                    painter.drawPixmap((150-w)/2, (114-h)/2, newImage)
                    painter.drawText(QtCore.QPoint(
                        (150-true_len*4.5)/2, 128), mytext[0])
                    painter.drawText(QtCore.QPoint(
                        (150-true_len*4.5)/2, 146), mytext[1])

                else:
                    if h > 132:  # On a besoin de 18 pixel pour ecrire une chaine de charactere
                        if(w/h)*114 <= 150:
                            newImage = newImage.scaledToHeight(
                                132, QtCore.Qt.SmoothTransformation)
                            w, h = newImage.width(), newImage.height()
                        else:
                            newImage = newImage.scaledToWidth(
                                150, QtCore.Qt.SmoothTransformation)
                    ...
                    painter.drawText(QtCore.QPoint(
                        (150-true_len*9)/2, 146), text)

                # On a une liste pour chaque type d objet #
                self.inter.append(ImageResult)
    ...
    def add_new_element(self, k, item):
        ...
        # On recupere le numero de la ligne dans la feuille excel
        col = liste[k].widget().row(item)
        length = 0
        instru_depth = 0
        anchor_weight = 0
        # Si c est un cable on demande sa longueur
        if self.tab_name_class[k] == "Ropes":
        ...
        # Si c est un instrument on demande sa profondeur
        if self.tab_name_class[k] == "Instruments":
        ...
        if self.mooringWidget.insert == 0:  # Pas d insertion
            # On demande la profondeur de la bouee de tetes
            if len(self.mooringWidget.liste_element) == 0:
                self.mooringWidget.ask_instru_depth(0)
                instru_depth = self.mooringWidget.instru_depth_value
                if instru_depth.toFloat()[1] == True:
                    instru_depth = instru_depth.toFloat()[0]
            # On ajoute dans les listes les informations entrees par lutilisateur
            self.mooringWidget.liste_element.append([k, col])
            self.mooringWidget.ropes_length.append(length)
            self.mooringWidget.clamp_ratio.append(
                2)            # 2 : Pas de clampage
            self.mooringWidget.instru_depth.append(instru_depth)
            self.mooringWidget.update_grid(0)  # Mis a jour du mouillage

        else:  # Insertion
            # En cas d insertion en premiere position on demande la profondeur de la bouee de tete
            if self.mooringWidget.insert_pos+1 == 0:
                self.mooringWidget.ask_instru_depth(0)
                instru_depth = self.mooringWidget.instru_depth_value
                if instru_depth.toFloat()[1] == True:
                    instru_depth = instru_depth.toFloat()[0]
            self.mooringWidget.liste_element.insert(
                self.mooringWidget.insert_pos+1, [k, col])
            self.mooringWidget.ropes_length.insert(
                self.mooringWidget.insert_pos+1, length)
            self.mooringWidget.instru_depth.insert(
                self.mooringWidget.insert_pos+1, instru_depth)
            self.mooringWidget.clamp_ratio.insert(
                self.mooringWidget.insert_pos+1, 2)
    ...
    def refresh_library(self):
        ...
        for i in range(len(self.mooringWidget.liste_element)):
            if type(self.mooringWidget.liste_element[i][0]) == int:
                a.append(self.tab_object[self.mooringWidget.liste_element[i]
                                         [0]][self.mooringWidget.liste_element[i][1]].name)
            else:
        ...
        for i in range(len(self.tab_name_class)):
            ...
            for p in range(rownum+1, a.nrows):
                ...
                for m in range(colnum+1, a.ncols):
                    ...
                    setattr(obj, a.col_values(m)[
                            rownum], str(a.col_values(m)[p]))
