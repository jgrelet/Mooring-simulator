...
class Simulate_wind(QtGui.QWidget):
    ...
    def __init__(self, data, parent=None):
        ...
        if float(eval(self.data[0][-1].mass))>0:                                 #Corrige la saisie de lutilisateur en cas de saisie positive du poids du lest
            self.data[0][-1].mass=str(-1*float(eval(self.data[0][-1].mass)))
            
        self.inventory=self.calculate_inventory()                                #Calcul l inventaire des elements utilise sur la ligne
        self.static_depth=self.calculate_static()                                #Calcul des profondeurs en statique, sans courant, ni allongement
        self.T,self.V_chute_t,self.T_max=self.resolve_max_Tension()              #Calcul du temps, de la vitesse de chute et des tensions max lors de cette chute
        self.percent_max_T=[]
        self.weight_kg=[]
        self.old_depth=[]
        self.max_value_anchor=self.Anchor_max_value()                            #Calcul du poids minimal,ideal,maximal pour le lest
        ...
        for i in range(len(self.T_max)):
            if type(self.data[0][i])!=list:
                if hasattr(self.data[0][i],"breaking_strength"):
                    self.percent_max_T.append(100*self.T_max[i]/(9.81*eval(self.data[0][i].breaking_strength)))
                else:
                    self.percent_max_T.append(0.0)
                self.weight_kg.append(eval(self.data[0][i].mass))   #On recupere egalement le poids de chaque element en kg
            else:
                if hasattr(self.data[0][i][0],"breaking_strength"):
                    self.percent_max_T.append(100*self.T_max[i]/(9.81*eval(self.data[0][i][0].breaking_strength)))
                else:
                    self.percent_max_T.append(0.0)
                self.weight_kg.append(eval(self.data[0][i][0].mass)+eval(self.data[0][i][1].mass))
                
        self.get_buoyancy_kg()                                   #On calcul la flottabilite en cas de rupture de la partie superieure en kg
       ...
       #On decoupe les cables en troncon de longueur minimale : cable de 100m = 10 x cable de 10 m
        while i<len(self.data[0]):
            if type(self.data[0][i])!=list:
                if eval(self.data[0][i].length)>l_max:
                    ...
                    new_projected_area=str(eval(self.data[0][i].projected_area)/coeff)
                    self.data[0][i].length=new_length
                    self.data[0][i].mass=new_mass
                    self.data[0][i].projected_area=new_projected_area
                    
                    for j in range(int(coeff)-1):                          #on ajoute le nouveau troncon en position i+1
                        self.data[0].insert(i+1,copy.deepcopy(self.data[0][i]))
                ...
                if eval(self.data[0][i][0].length)>l_max:
                    ...
                    new_projected_area=str(eval(self.data[0][i][0].projected_area)/coeff)
                    ...
                    for j in range(int(coeff)-1):
                        self.data[0].insert(i+1,copy.deepcopy(self.data[0][i][0]))
                        self.data[2].insert(i+1,0)
                        count=count+1
            i=i+1
        ...
        self.make_table(alpha,F)                                        #Genere le tableau recapitulatif de la simulation
        ...
        for i in range(len(self.data[0])):
            if type(self.data[0][i])!=list:
                ...
                if self.data[0][i].__class__.__name__=="Ropes":
                    inventory_length[ind]=inventory_length[ind]+eval(self.data[0][i].length)
                else:
                ...
                if self.data[0][i][0].__class__.__name__=="Ropes":
                    inventory_length[ind]=inventory_length[ind]+eval(self.data[0][i][0].length)
                else:
                    inventory_length[ind]=inventory_length[ind]+1
                ind=inventory_name.index(self.data[0][i][1].name)
                if self.data[0][i][1].__class__.__name__=="Ropes":
                    inventory_length[ind]=inventory_length[ind]+eval(self.data[0][i][1].length)
                else:
                    inventory_length[ind]=inventory_length[ind]+1
    ...
    def calculate_static(self):
        ...
        for b in self.data[0]:
            if type(b)!=list:
                ...
                self.original_length.append([round(eval(b[0].length),1),round(eval(b[1].length),1)])
    ...
    def resolve_max_Tension(self):
            ...
            for j in range(i+1):
                if j==len(self.data[0]):
                    ...
                    if type(self.data[0][j])!=list:
                        ...
                        Tension[i]=Tension[i]+g*(eval(self.data[0][j][0].mass)+eval(self.data[0][j][1].mass))
        
        a=(self.data[3][-1])/(-mass_lest)               #Trainee induite par les elements superieures au lest
        ...
        if b>0:
            ...
            for i in range(len(Tension)):
                T_max.append(0)
                T_max[i]=(self.data[3][i]*(v_chute**2)+Tension[i])  #Calcul des Tensions max
            T_max[-1]=0.0
            return  T,V_chute_t,T_max
        else:
             QtGui.QMessageBox.warning(self,'Message',"Your Anchor is too light")
    ...
    def Anchor_max_value(self):
        ...
        for i in range(len(self.data[0])):
            if type(self.data[0][i])!=list:
                if hasattr(self.data[0][i],"breaking_strength"):
                    v_chute=sqrt((eval(self.data[0][i].breaking_strength)*g-Tension[i])/( self.data[3][i]))
                    vec_v_chute.append(v_chute)  
            else:
                if hasattr(self.data[0][i][0],"breaking_strength"):
                    v_chute=sqrt((eval(self.data[0][i][0].breaking_strength)*g-Tension[i])/( self.data[3][i]))
                    vec_v_chute.append(v_chute)
                    
        mass_max=(Tension[-2]+self.data[3][-1]*(min(vec_v_chute) )**2)/g    #On prend l element qui supporte la vitesse de chute la plus petite, cas le plus defavorable
        return mass_max
    ...
    def get_buoyancy_kg(self):
        ...
        for i in range(len(self.data[0])):
            if(self.data[0][i].__class__.__name__)=="Releases":            #On recupere l indice du largueur car ce calcule nest pas pertinent en dessous de celui-ci
                release_ind=i
    ...
    def current_vector(self,depth):
        ...
        for i in range(len(depth)):
            ...
            while(depth[i]<self.data[1][1][a]):
                a=a+1
            k=(self.data[1][0][a-1]-self.data[1][0][a])/(self.data[1][1][a-1]-self.data[1][1][a])
    ...
    def update_depth(self,depth,alpha):
        ...
        for i in range(2,len(self.data[0])+2):
            depth.insert(0,0)
            if type(self.data[0][-i+1])!=list:
                depth[-i]=depth[-i+1]+cos(alpha[-i+1])*eval(self.data[0][-i+1].length)
            else:
               depth[-i]=depth[-i+1]+cos(alpha[-i+1])*eval(self.data[0][-i+1][0].length)
        depth.remove(self.data[1][1][len(self.data[1][0])-1])
        
        #Calcul de la profondeur au milieu de l element#
        for i in range(len(self.data[0])):
            if type(self.data[0][i])!=list:
                depth[i]=depth[i]-cos(alpha[i])*eval(self.data[0][i].length)/2
            else:
                depth[i]=depth[i]-cos(alpha[i])*eval(self.data[0][i][0].length)/2
    ...
    def resolve(self):
        ...
        for i in range(len(self.depth)):
            ...
            if type(self.data[0][i])!=list:
                ...
                inter=[eval(self.data[0][i][0].mass)*g,eval(self.data[0][i][1].mass)*g]
                P.append(inter)
        ...
        while (cond==1):                                                #Tant que le systeme nest pas stable
            ...
            for i in range(len(alpha)):                                 #On met a jour la valeur de alpha
                alpha[i]=alpha_new[i]
            new_depth=self.update_depth(new_depth,alpha)                #On met a jour le vecteur profondeur
            V=self.current_vector(new_depth)                            #On met a jour le vecteur de courant
            #On commence par la tete de mouillage#
            Tn[0]=0.5*rho*eval(self.data[0][0].nl_drag_cf)*eval(self.data[0][0].projected_area)*(V[0]*cos(alpha[0]))**2     #Calcul trainee normal 
            Tt[0]=0.5*rho*eval(self.data[0][0].tl_drag_cf)*eval(self.data[0][0].projected_area)*(V[0]*sin(alpha[0]))**2     #Calcul trainee tangentielle
            Fx[0]=Tn[0]*cos(alpha[0])+Tt[0]*sin(alpha[0])                                                                   #Resultante selon x
            Fy[0]=Tt[0]*cos(alpha[0])-Tn[0]*sin(alpha[0])+P[0]                                                              #Resultante selon y
            ...
            for i in range(1,len(self.depth)-1):
                if type(self.data[0][i])!=list:
                    Tn[i]=0.5*rho*eval(self.data[0][i].nl_drag_cf)*eval(self.data[0][i].projected_area)*(V[i]*cos(alpha[i]))**2 #Calcul trainee normal 
                    Tt[i]=0.5*rho*eval(self.data[0][i].tl_drag_cf)*eval(self.data[0][i].projected_area)*(V[i]*sin(alpha[i]))**2 #Calcul trainee tangentielle
                    Fx[i]=Tn[i]*cos(alpha[i])+Tt[i]*sin(alpha[i])+Fx[i-1]                                                       #Resultante selon x
                    Fy[i]=Tt[i]*cos(alpha[i])-Tn[i]*sin(alpha[i])+P[i]+Fy[i-1]                                                  #Resultante selon y
                ...
                #ELement clampe#
                    ...
                    Fx[i]=Tn[i]*cos(alpha[i])+Tt[i]*sin(alpha[i])+Fx[i-1]                                                                       #Resultante selon x
                    Fy[i]=Tt[i]*cos(alpha[i])-Tn[i]*sin(alpha[i])+P[i][0]+P[i][1]+Fy[i-1]                                                       #Resultante selon y
                    alpha_new[i]=fact_convergence*atan(Fx[i]/Fy[i])                                                                             #Angle
                    F[i]=sqrt(Fx[i]**2+Fy[i]**2)                                                                                                #Norme
    ...
    def calculate_stretch(self,F):
        ...
        for i in range(len(self.data[0])):
            ...
            if type(self.data[0][i])!=list:
                if self.data[0][i].__class__.__name__=="Ropes":
                    ratio=100/9.81*(F[i]/eval(self.data[0][i].breaking_strength)) #On calcule  le pourcentage de la charge de rupture qui est applique au cable
                    new_stretch=(round(eval(self.data[0][i].poly_1),2)*ratio**2+round(eval(self.data[0][i].poly_2),2)*ratio+round(eval(self.data[0][i].poly_3),2))/100 
                    ...
                    self.data[0][i].length=str(eval(self.data[0][i].length)*(1+new_stretch)) #On met a jour la nouvelle longueur
            #Element clampe#
            else:
                if self.data[0][i][0].__class__.__name__=="Ropes":
                    ratio=100/9.81*(F[i]/eval(self.data[0][i][0].breaking_strength))
                    new_stretch=(round(eval(self.data[0][i][0].poly_1),2)*ratio**2+round(eval(self.data[0][i][0].poly_2),2)*ratio+round(eval(self.data[0][i][0].poly_3),2))/100
                    self.data[0][i][0].length=str(eval(self.data[0][i][0].length)*(1+new_stretch))
    ...
    def calculate_ratio(self,alpha):
        ...
        for i in range(2,len(self.data[0])+2):
            ...
            x_float.insert(0,0)
            if type(self.data[0][-i+1])!=list:
                y_float[-i]=y_float[-i+1]+cos(alpha[-i+1])*eval(self.data[0][-i+1].length)
                x_float[-i]=x_float[-i+1]+sin(alpha[-i+1])*eval(self.data[0][-i+1].length)
            else:
               y_float[-i]=y_float[-i+1]+cos(alpha[-i+1])*eval(self.data[0][-i+1][0].length)
               x_float[-i]=x_float[-i+1]+sin(alpha[-i+1])*eval(self.data[0][-i+1][0].length)
        y_float.remove(self.data[1][1][len(self.data[1][0])-1])
        x_float.remove(x_float[-1])
        self.myxfloat=x_float
        self.myyfloat=y_float
        self.x_instru=[x_float[0]-sin(alpha[0])*eval(self.data[0][0].length)/2]             #Coordonnee x de la bouee de tete
        self.y_instru=[y_float[0]-cos(alpha[0])*eval(self.data[0][0].length)/2]             #Coordonnee y de la bouee de tete
        self.x_instru_top=[]
        self.y_instru_top=[]
        ...
        for i in range(len(self.data[6])):
            if self.data[6][i]==0.0:                                                        #Ratio defini comme auto
                calc=0
                if self.data[2][self.find_match(i)][1]>self.myyfloat[self.find_match(i)]:   #Si la profondeur choisi par lutilisateur est plus haut que le support de clampage
                    self.data[6][i]=0                                                        #le ratio est fixe au minimum c est a dire 0
                    calc=1
                if self.data[2][self.find_match(i)][1]<self.myyfloat[self.find_match(i+1)]:  #Si la profondeur choisi par lutilisateur est plus basse que le support de clampage
                    self.data[6][i]=1                                                        #le ratio est fixe au minimum c est a dire 1
                    calc=1
                if calc==0:
                    malongueur=eval(self.data[0][self.find_match(i)][0].length)
                    lon=0
                    p=1
                    ...
                    while(self.myyfloat[self.find_match(i)+p]>self.data[2][self.find_match(i)][1]):
                        if type(self.data[0][self.find_match(i)+p-1])!=list:
                            lon=lon+eval(self.data[0][self.find_match(i)+p-1].length)
                        else:
                            lon=lon+eval(self.data[0][self.find_match(i)+p-1][0].length) 
                        p=p+1
                ...
                if self.data[6][i]>=1 or self.data[6][i] <=0:
                    QtGui.QMessageBox.warning(self,'Message',"An instrument is clamped beyond it support")
        ...
        for i in range(len(self.data[6])):
            if type(self.data[0][self.find_match(i)])!=list:
                if (self.data[0][self.find_match(i)]).__class__.__name__=="Instruments":
                    self.name_instru.append(self.data[0][self.find_match(i)].name)
                    self.x_instru.append(self.myxfloat[self.find_match(i)]-0.5*eval(self.data[0][self.find_match(i)].length)*sin(alpha[self.find_match(i)]))
                    self.y_instru.append(self.myyfloat[self.find_match(i)]-0.5*eval(self.data[0][self.find_match(i)].length)*cos(alpha[self.find_match(i)]))
                    self.x_instru_top.append(self.myxfloat[self.find_match(i)])                                                                                     #coordonnee x en haut de l instrument
                    self.y_instru_top.append(self.myyfloat[self.find_match(i)])                                                                                     #coordonnee y en haut de l instrument
        #Element clampe#
            else:
                if (self.data[0][self.find_match(i)][1]).__class__.__name__=="Instruments":
                    self.name_instru.append(self.data[0][self.find_match(i)][1].name)
                    malongueur=eval(self.data[0][self.find_match(i)][0].length)
                    ...
                    while(lon<malongueur*self.data[6][i]):
                        ...
                        p=p+1
                        lon=lon+eval(self.data[0][int(self.table_match[i])+p].length)
                        lon2=eval(self.data[0][int(self.table_match[i])+p].length)

                    self.x_instru.append(self.myxfloat[self.find_match(i)+p]-ratio*malongueur*sin(alpha[self.find_match(i)+p]))
                    self.y_instru.append(self.myyfloat[self.find_match(i)+p]-ratio*malongueur*cos(alpha[self.find_match(i)+p]))
                    self.x_instru_top.append(self.myxfloat[self.find_match(i)+p]-(ratio*malongueur-0.5*eval(self.data[0][self.find_match(i)][1].length))*sin(alpha[self.find_match(i)+p])) #coordonnee x en haut de l instrument
                    self.y_instru_top.append(self.myyfloat[self.find_match(i)+p]-(ratio*malongueur-0.5*eval(self.data[0][self.find_match(i)][1].length))*cos(alpha[self.find_match(i)+p])) #coordonnee y en haut de l instrument
    ...
    def plot_figure(self,alpha,P,F):
        ...
        safe_value_anchor=(1.5/9.81)*(F[-2]*(cos(alpha[-2])+0.6*sin(alpha[-2])))        #Valeur safe lest formule visbeck runmoor
        self.anchor_value=[round(safe_value_anchor,1),-1*round(float(self.data[0][-1].mass),1),round(max(self.percent_max_T),1)]
        ...
        label1.setStyleSheet("QLabel { background-color : white; color : black;border: 2px solid black }")
        label2=QtGui.QLabel("Min")
        label2.setStyleSheet("QLabel { background-color : white; color : black;border: 2px solid black }")
        label3=QtGui.QLabel("Max")
        label3.setStyleSheet("QLabel { background-color : white; color : black;border: 2px solid black }")
        label4=QtGui.QLabel(str(round(min_value_anchor,1)))
        label4.setStyleSheet("QLabel { background-color : white; color : black;border: 1px solid black }")
        label5=QtGui.QLabel(str(round(self.max_value_anchor,1)))
        label5.setStyleSheet("QLabel { background-color : white; color : black;border: 1px solid black }")
        label6=QtGui.QLabel("Safe Anchor's weight")
        label6.setStyleSheet("QLabel { background-color : white; color : black;border: 2px solid black }")
        label7=QtGui.QLabel("Selected Anchor's weight")
        label7.setStyleSheet("QLabel { background-color : white; color : black;border: 2px solid black }")
        label8=QtGui.QLabel(str(self.anchor_value[0]))
        label8.setStyleSheet("QLabel { background-color : white; color : black;border: 1px solid black }")
        label9=QtGui.QLabel(str(self.anchor_value[1]))
        if self.anchor_value[1]<self.anchor_value[0]:                                                       #Valeur choisie < valeur recommande
            label9.setStyleSheet("QLabel { background-color : red; color : white ;border: 2px solid black }")
        else:
            label9.setStyleSheet("QLabel { background-color : green; color : white;border: 1px solid black }")
            
        label10=QtGui.QLabel("Max Launch Tension / Breaking Strength (%)")
        label10.setStyleSheet("QLabel { background-color : white; color : black;border: 2px solid black }")
        label11=QtGui.QLabel(str(self.anchor_value[2]))
        if round(max(self.percent_max_T),1)>40:                                                             #T_max/Ultimate_load >40%
            label11.setStyleSheet("QLabel { background-color : red; color : white ;border: 2px solid black }")
        else:
            label11.setStyleSheet("QLabel { background-color : green; color : white;border: 1px solid black }")  
    ...
    def make_table(self,alpha,F):
        ...
        for i in range(len(self.table_match)):
            for j in range(i,int(self.table_match[i])-count):
                if type(self.data[0][i-1])!=list:
                        self.data[0][i-1].length=str(eval(self.data[0][i-1].length)+eval(self.data[0][i].length))
                        self.data[0][i-1].mass=str(eval(self.data[0][i-1].mass)+eval(self.data[0][i].mass))
                        self.data[0][i-1].projected_area=str(eval(self.data[0][i-1].projected_area)+eval(self.data[0][i].projected_area))
                else:
                        self.data[0][i-1][0].length=str(eval(self.data[0][i-1][0].length)+eval(self.data[0][i].length))
                        self.data[0][i-1][0].mass=str(eval(self.data[0][i-1][0].mass)+eval(self.data[0][i].mass))
                        self.data[0][i-1][0].projected_area=str(eval(self.data[0][i-1][0].projected_area)+eval(self.data[0][i].projected_area))      
                del(self.data[0][i])
                del(alpha[i])
                del(F[i])
                del(self.myxfloat[i])
        ...
        for p in self.data[0]:
            if type(p)!=list:
                ...
                name.append(p[0].name+' '+'+'+' '+p[1].name)             #Recuperation des noms
        ...
        for i in range(len(self.data[0])):
            ...
            if type(self.data[0][i])!=list:
                ...
                if self.data[0][i].__class__.__name__=="Instruments":   #Detection des instruments
                ...
                if self.data[0][i].__class__.__name__=="Ropes": 
                    length_stretch.append(str(round(eval(self.data[0][i].length),1))+' '+'('+str(round(self.percent_stretch[i],1))+'%'+')') #Recuperation des longueurs allongees
                else:
     ...
     #Element clampe#            
                ...
                if self.data[0][i][1].__class__.__name__=="Instruments":        
                    ind_instrum.append(i)
                    my_prof.append(str(round(self.myyfloat[i],1))+' '+'+'+' '+str(round(self.y_instru_top[count],1))+' '+'ratio = '+str(round(self.data[6][i],2))) #Affichage profondeur support+profondeur element clampe+clamp_ratio
                    if self.data[6][i]>=1 or self.data[6][i]<=0:
                        warn.append(i)
                    count=count+1
                else:
                     my_prof.append(str(round(self.myyfloat[i],1)))
                if self.data[0][i].__class__.__name__=="Ropes":
                    length_stretch.append(self.data[0][i][0].length+' '+'('+self.percent_stretch[i]+')')
                else:
                    length_stretch.append('')
        ...
        Tab_final.insert(0,["Name","Static Length (m)","Depth (m)","Tension (Kp)","Angle (deg)","Dx (m)","Dz (m)","Buoy (kg)","Weight (kg)","Launch Tension (%)", "Length Stretched (m)"])
        ...
        for k in range(1,len(Tab_final)):           
            if self.data[0][k-1].__class__.__name__!="Terminals":       #Affichage de chaque element sauf terminaison
                inter=[]
                for l in range(len(Tab_final[k])):
                    ...
                    if k-1 in ind_instrum or k==1 or k-1==release_ind or k==len(Tab_final)-1:  #On repere en bleu ciel : la bouee de tete, les instruments, les largueurs, et le lest
                        label.setStyleSheet("QLabel { background-color : rgb(168, 211, 255); color : black;border: 1px solid black }")
                        if k-1 in warn:                                                         #Un instrument clampe au dela de son support
                            label.setStyleSheet("QLabel { background-color : red; color : black;border: 1px solid black }")
