...
class Library_wind(QtWidgets.QWidget):
    ...
    def __init__(self,name):
        ...
        self.myLayout_library=QtWidgets.QVBoxLayout()
        mypath=getcwd()     #Recuperation du repertoire courant
        #Bouton refresh
        refresh_library=QtWidgets.QAction(QtGui.QIcon(mypath+'\Library\Pictures\Icons\Refresh.png'), 'Refresh_library', self)
        refresh_library.triggered.connect(functools.partial(self.refresh_library,name))
        #Bouton launch
        launch_library=QtWidgets.QAction(QtGui.QIcon(mypath+'\Library\Pictures\Icons\Spreadsheet.png'), 'Launch_library', self)
    ...
    def create_label(self):
        ...
        myarea=QtWidgets.QMdiArea(self)
        ...
        for i in range(len(self.tab_name_class)):
            ...
            for j in range(len(self.tab_attr[i])): 
                label=QtWidgets.QLabel(self.tab_attr[i][j])
            ...
            for k in range(len(self.tab_object[i])):
                for l in range(len(self.tab_attr[i])):
                    label=QtWidgets.QLabel(getattr(self.tab_object[i][k],self.tab_attr[i][l]))
