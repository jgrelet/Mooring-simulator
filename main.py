...
class Mooring_Simulator(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        #Action#
        self.new_mooring_Action = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'New Mooring', self)
        self.new_mooring_Action.triggered.connect(
            self.new_mooring_pushButton_clicked)

        self.load_mooring_Action = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'Load Mooring', self)
        self.load_mooring_Action.triggered.connect(
            self.load_mooring_pushButton_clicked)

        self.save_mooring_Action = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'Save Mooring', self)
        self.save_mooring_Action.triggered.connect(
            self.save_mooring_pushButton_clicked)

        self.exitAction = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'Exit', self)
        self.exitAction.setShortcut('Ctrl+Q')
        self.exitAction.setStatusTip('Exit application')
        self.exitAction.triggered.connect(self.close)

        self.open_library_Action = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'Show Library', self)
        self.open_library_Action.triggered.connect(
            self.library_pushButton_clicked)

        self.load_library_Action = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'Load new Library', self)
        self.load_library_Action.triggered.connect(
            self.load_library_pushButton_clicked)


#       self.edit_preference_Action = QtWidgets.QAction(QtGui.QIcon('exit24.png'),'Preferences', self)
#       self.edit_preference_Action.triggered.connect(self.edit_preference_clicked)
#

        self.edit_environnement_Action = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'Set Environnemental Conditions', self)
        self.edit_environnement_Action.triggered.connect(
            self.edit_environnement_pushButton_clicked)

        self.start_simulation_Action = QtWidgets.QAction(QtGui.QIcon(
            self.mypath+'\Library\Pictures\Icons\play.png'), 'Start simulation', self)
        self.start_simulation_Action.triggered.connect(
            self.start_simulation_pushButton_clicked)

        self.generate_report_Action = QtWidgets.QAction(
            QtGui.QIcon('exit24.png'), 'Generate report', self)
        self.generate_report_Action.triggered.connect(
            self.generate_report_pushButton_clicked)
    ...
    def create_mooring_wind(self, file_name):
        ...
        self.mooring_wind = MooringWindow(
            self.Defaut_folder, self.screen_width)
    ...
    def load_mooring_pushButton_clicked(self):
        ...
        # l utilisateur va chercher a l aide de l explorer le fichier de sauvegarde
        (fileName, _) = QtWidgets.QFileDialog.getOpenFileName(
            self, ("Open File"), "C:\git\Python\Mooring-simulator\examples", ("Mooring (*.py)"))
        # DEBUG !!!!!! hardcoded
        #fileName = "C:\git\Python\Mooring-simulator\examples\\test1.py"
        ...
        while data[i] != "depth_instru\n":
            ...
            if clamp == 0:
                ...
                while l < len(data[i]):
                    ...
                    l = l+1
                # On recupere la longueur des item
                list_length.append(eval(length_item))
            #clampage#
            if clamp == 1:
                while data[i][j] != " ":
                ...
                # On recupere la longueur du support
                list_length.append(eval(length_item))
                leng = ""
                while data[i][ind] != " ":
                ...
                while(data[i][ind+1+eval(leng)+z]) != ';':
                    ...
                    z = z+1
                # On recupere le ratio de clampage
                clamp_ratio.append(float(clamp_rat))
            i = i+1
        i = i+1
        list_depth_instru = []
        ...
        while data[i] != "fin\n":
            ...
            for u in range(len(data[i])-1):
                ...
                if data[i][u] == '_':
                    clamp = 1
            if clamp != 1:
                # Recupere la profondeur des elements non clampe
                list_depth_instru.append(eval(depth))
                ...
                inter.append(eval(depth))
                # Recupere la profondeur des elements clampes
                list_depth_instru.append(inter)
            i = i+1
                ...
                for j in range(len(self.mooring_wind.tab_object)):
                    for k in range(len(self.mooring_wind.tab_object[j])):
                        if self.mooring_wind.tab_object[j][k].name == list_item[i]:
                            self.mooring_wind.mooringWidget.liste_element.append([
                                                                                 j, k])
            else:
                temp = []
        ...
        for i in range(len(self.environnement_wind.liste_point)):
            self.environnement_wind.remove_current_point(
                self.environnement_wind.liste_point[0])
        ...
        # On appelle la fonction du combobox
        self.environnement_wind.combo_onActivated()
        if (data[-1]) != "None":
            self.environnement_wind.save_surf_bott_btn(
                1)  # On sauvegarde les parametre
        if len(current_point) != 0:
            #On met a jour les valeurs et profondeur de courant du fond et de la surface#
            self.environnement_wind.label_current7.setText(
                current_point[0].split(';')[0])
            self.environnement_wind.line1.setText(
                current_point[0].split(';')[1])
            self.environnement_wind.label_current5.setText(
                current_point[-1].split(';')[0])
            self.environnement_wind.line2.setText(
                current_point[-1].split(';')[1])
            ...
            for i in reversed(range(1, len(current_point)-1)):
                ...
                self.environnement_wind.mylayout.itemAtPosition(
                    3, 1).widget().setText(current_point[i].split(';')[0])
                self.environnement_wind.mylayout.itemAtPosition(
                    3, 2).widget().setText(current_point[i].split(';')[1])
            # self.environnement_wind.update()
        # On met a jour le layout de la ligne de mouillage
    ...
    def save_mooring_pushButton_clicked(self):
        ...
        fileName = QtWidgets.QFileDialog.getSaveFileName(
            self, ("Save File"), "/home", ("Mooring (*.py)"))  # On recupere dans Filename le chemin du fichier de sauvegarde
        self.create_save(fileName)  # Creation du fichier de sauvegarde
        filename = self.get_file_name(fileName)
        # Le titre de l onglet venant d etre sauvegarde prend le nom entre par l utilisateur
        self.zoneCentrale.setTabText(
            self.zoneCentrale.currentIndex(), filename)
    ...
    def create_save(self, fileName):
        ...
        for i in range(len(self.mooring_wind.mooringWidget.liste_element)):
            if type(self.mooring_wind.mooringWidget.liste_element[i][0]) != list:
                ...
                fichier.write(str(len(self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[i][0]][self.mooring_wind.mooringWidget.liste_element[i][1]].name))
                              + " "+self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[i]
                                                                 [0]][self.mooring_wind.mooringWidget.liste_element[i][1]].name
                ...
                fichier.write(str(len(self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[i][0][0]][self.mooring_wind.mooringWidget.liste_element[i][0][1]].name))+" " +
                              ...
                              str(len(self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[i]
                                                                   [1][0]][self.mooring_wind.mooringWidget.liste_element[i][1][1]].name))+" "
        ...
        for i in range(len(self.mooring_wind.mooringWidget.liste_element)):
            if type(self.mooring_wind.mooringWidget.instru_depth[i]) != list:
                fichier.write(
                    str(self.mooring_wind.mooringWidget.instru_depth[i])+"\n")
            else:
                fichier.write(str(self.mooring_wind.mooringWidget.instru_depth[i][0])+"_"+str(
                    self.mooring_wind.mooringWidget.instru_depth[i][1])+"\n")
        fichier.write("fin\n")
        fichier.write("\n")
        # Recupere les valeurs et profondeur de courant
        y, x = self.environnement_wind.get_all_current_value()
        ...
        # Poids du lest
        fichier.write(
            str(self.mooring_wind.mooringWidget.anchor_weight_value)+';'+'\n')
        # Index du combo definissant la maniere de definir les profondeurs
        fichier.write(
            str(self.environnement_wind.combo_1.currentIndex())+';'+'\n')
        # Profondeur du fond/surface
        fichier.write(self.environnement_wind.choice_depht_height.text()+';')
    ...
    def new_mooring_pushButton_clicked(self):
        ...
        for i in range(len(self.environnement_wind.liste_point)):
            # On supprime les anciens points de courant
            self.environnement_wind.remove_current_point(
                self.environnement_wind.liste_point[0])
        self.environnement_wind.choice_depht_height.setText(
            "None")  # On reinitialise la profondeur du fond
    ...
    def load_library_pushButton_clicked(self):
        ...
        fileName = QtWidgets.QFileDialog.getOpenFileName(
            self, ("Open File"), "/home", ("Spreadsheet  (*.xls)"))
...
#       self.preference_dockwidget.show()
    ...
    def start_simulation_pushButton_clicked(self):
        ...
        if self.environnement_wind.param_saved == 1:  # On verifie que les parametres environnementals ont ete sauvegarde
            ...
            QtWidgets.QMessageBox.information(
                self, 'Message', "Choose environemental conditions before starting simulation")
    ...
    def estimate_length(self):
        ...
        for a in self.mooring_wind.mooringWidget.clamp_ratio:
            # On sauvegarde les valeurs d origines du clamp ratio
            self.mooring_wind.mooringWidget.clamp_ratio2.append(a)
        # On verifie que l utilisateur a entre des valeurs coherentes pour les profondeurs des instruments
        for i in range(len(self.mooring_wind.mooringWidget.instru_depth)):
            if type(self.mooring_wind.mooringWidget.instru_depth[i]) != list:
                if self.environnement_wind.combo_1.currentIndex() == 0:
                    if self.mooring_wind.mooringWidget.instru_depth[i] < 0:
                        # En cas d erreur de saisie on modifie la valeur
                        self.mooring_wind.mooringWidget.instru_depth[
                            i] = self.mooring_wind.mooringWidget.instru_depth[i]*-1
                ...
                for j in range(len(self.mooring_wind.mooringWidget.instru_depth[i])):
                    if self.environnement_wind.combo_1.currentIndex() == 0:
                        if self.mooring_wind.mooringWidget.instru_depth[i][j] < 0:
                            self.mooring_wind.mooringWidget.instru_depth[i][
                                j] = self.mooring_wind.mooringWidget.instru_depth[i][j]*-1
                    if self.environnement_wind.combo_1.currentIndex() == 1:
                        if self.mooring_wind.mooringWidget.instru_depth[i][j] > 0:
                            self.mooring_wind.mooringWidget.instru_depth[i][
                                j] = self.mooring_wind.mooringWidget.instru_depth[i][j]*-1

        floor = self.environnement_wind.combo_1.currentIndex(
        )*(-1*self.environnement_wind.choice_depht_height.text().toFloat()[0])  # Profondeur du fond
        depth = []
        depth.append(floor)
        my_depth = 0
        mylength = []
        clamp = 0
        a = 1
        # On recupere les longueurs de chaque element
        for i in range(len(self.mooring_wind.mooringWidget.ropes_length)):
            if type(self.mooring_wind.mooringWidget.instru_depth[i]) != list:
                myobject = self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[i]
                                                        [0]][self.mooring_wind.mooringWidget.liste_element[i][1]]
            else:
                myobject = self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[i]
                                                        [0][0]][self.mooring_wind.mooringWidget.liste_element[i][1][0]]
            if hasattr(myobject, "length"):
                mylength.append(eval(myobject.length))

            else:
                mylength.append(
                    float(self.mooring_wind.mooringWidget.ropes_length[i]))
        if 0.0 in mylength:  # Cela veut dire que ce cable est defini en auto
            for i in reversed(range(len(mylength))):
                if mylength[i] != 0:
                    # On calcule la profondeur jusqu'a la longueur inconnue
                    depth.insert(0, depth[0]-mylength[i])
                else:
                    inter = 0
                    for j in reversed(range(i)):
                        # On calcule la longueur entre le cable defini en automatique et l instrument
                        inter = inter+mylength[j]
                        if type(self.mooring_wind.mooringWidget.instru_depth[j]) != list:
                          # Pas de clampage
                            if self.mooring_wind.mooringWidget.instru_depth[j] != 0:
                                # On recupere la profondeur de l instrument
                                my_depth = self.mooring_wind.mooringWidget.instru_depth[j]
                                clamp = 0
                                break
                            else:
                                my_depth = 0
                          ...
                          # Clampage
                            for k in range(len(self.mooring_wind.mooringWidget.instru_depth[j])):
                                # On verifie que le ratio de clampage est bien definie
                                if self.mooring_wind.mooringWidget.clamp_ratio[j] != 2 or self.mooring_wind.mooringWidget.clamp_ratio[j] != 0:
                                    if self.mooring_wind.mooringWidget.instru_depth[j][k] != 0:
                                        # On recupere la profondeur de l instrument
                                        my_depth = self.mooring_wind.mooringWidget.instru_depth[j][k]
                                        longu = eval(self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[j][k][0]][
                                                     self.mooring_wind.mooringWidget.liste_element[j][k][1]].length)  # longueur de l instrument
                                        clamp = 1
                                        break
                                    else:
                                        my_depth = 0
                            if my_depth != 0:
                                break
                    if clamp == 1:
                        # On calcule ma_longueur pour placer le haut de instrument a la profondeur a my_depth
                        ma_longueur = depth[0]+(my_depth)+float(
                            self.mooring_wind.mooringWidget.clamp_ratio[j])*mylength[j]-longu/2-inter
                    else:
                        ma_longueur = depth[0]+(my_depth)-inter

                    if ma_longueur < 0:
                        QtWidgets.QMessageBox.warning(
                            self, 'Message', "Error your mooring is incorrectly designed some length are negatives")

                    mylength[i] = ma_longueur
                    # On affecte la longueur calcule
                    self.mooring_wind.mooringWidget.ropes_length[i] = ma_longueur
                    # On insere la nouvelle profondeur
                    depth.insert(0, depth[0]-mylength[i])
            self.mooring_wind.mooringWidget.update_grid(0)
    ...
    def gathering_mooring_information(self):
        ...
        for i in range(len(self.mooring_wind.mooringWidget.liste_element)):
            ...
            if type(self.mooring_wind.mooringWidget.liste_element[i][0]) == int:
                tab.append(copy.deepcopy(
                    self.mooring_wind.tab_object[self.mooring_wind.mooringWidget.liste_element[i][0]][self.mooring_wind.mooringWidget.liste_element[i][1]]))
            else:
        ...
        for i in range(len(tab)):
            ...
            for j in range(i+1):
                if j == len(tab):
                    ...
                    if type(tab[j]) != list:
                        drag_v[i] = drag_v[i]+0.5*1025 * \
                            eval(tab[j].projected_area)*eval(tab[j].tl_drag_cf)
                    else:
                        drag_v[i] = drag_v[i]+0.5*1025*(eval(tab[j][0].projected_area)*eval(
                            tab[j][0].tl_drag_cf)+eval(tab[j][1].projected_area)*eval(tab[j][1].tl_drag_cf))

        tab[-1].mass = str(self.mooring_wind.mooringWidget.mylayout.itemAtPosition(
            len(tab), 2).widget().text())  # On affecte le poids du lest
        # On sauvegarde la valeur
        self.mooring_wind.mooringWidget.anchor_weight_value = tab[-1].mass
        for i in range(len(tab)):
            if type(tab[i]) != list:
              #Element non clampe#
                if not(hasattr(tab[i], "length")):
                    tab[i].length = str(self.mooring_wind.mooringWidget.mylayout.itemAtPosition(
                        i+1, 2).widget().text())  # On affecte la longueur du cable
                    # On calcule la masse du cable
                    tab[i].mass = str(eval(tab[i].length)*eval(tab[i].mass))
                    # On calcule la surface du cable
                    tab[i].projected_area = str(
                        eval(tab[i].length)*eval(tab[i].projected_area))
                    self.mooring_wind.mooringWidget.ropes_length[i] = tab[i].length
            else:
              #Element clampe#
                if not(hasattr(tab[i][0], "length")):
                    tab[i][0].length = str(
                        self.mooring_wind.mooringWidget.mylayout.itemAtPosition(i+1, 2).widget().text())
                    tab[i][0].mass = str(
                        eval(tab[i][0].length)*eval(tab[i][0].mass))
                    tab[i][0].projected_area = str(
                        eval(tab[i][0].length)*eval(tab[i][0].projected_area))
        ...
        # On ajoute profondeur et valeur de courant
        data.append([x_float, y_float])
        # On ajoute les profondeurs des instruments
        ...
        # On ajoute la copie de clamp_ratio
        data.append(self.mooring_wind.mooringWidget.clamp_ratio2)
    ...
    def gathering_report_information(self):
        ...
        for i in range(len(self.mooring_wind.mooringWidget.image2)):
            ...
            # On ajoute la profondeur du fond
            if i == len(self.mooring_wind.mooringWidget.image2)-1:
                text = str(
                    self.environnement_wind.choice_depht_height.text())+''+'m'
    ...
    def generate_report_pushButton_clicked(self):
        ...
        if hasattr(self, "simulate_wind"):  # On verifie que le mouillage a bien ete simule
            ...
            QtWidgets.QMessageBox.information(
                self, 'Message', "Simulate your mooring before creating the report")
...
#        self.preference_dockwidget.setWindowTitle('Preferences')
    ...
    def create_environnement(self):
        ...
        self.environnement_dockwidget.setWindowTitle(
            'Set Environnemental Conditions')
        self.environnement_dockwidget.show()
    ...
    def closerequested(self, k):
        ...
        reply = QtWidgets.QMessageBox.question(self, 'Message',
                                               "Are you sure to close this window?", QtWidgets.QMessageBox.Yes |
