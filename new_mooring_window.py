...
class MooringWidget(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        #Fonction de zoom#
        zoom_in_Action = QtGui.QAction(QtGui.QIcon(
            self.mypath+'\Pictures\Icons\zoom_in.png'), 'Zoom_in', self)
        zoom_out_Action = QtGui.QAction(QtGui.QIcon(
        ...
        self.button_insert_first = QtGui.QPushButton(
    ...
    def update_grid(self, vbar_statut):
        ...
        for i in range(len(self.liste_element)):  # On parcourt la liste des elements
            ...
            if i == (len(self.liste_element)-1) and self.anchor_weight_value != '0':
                ...
                line_edit_weight = QtGui.QLineEdit(
    ...
    def create_mooring_image(self):
        ...
        for i in range(len(self.liste_element)):
            ...
            if not newImage.load(path):
                QtGui.QMessageBox.warning(
                    self, "Open Image", "The image file could not be loaded.", QtGui.QMessageBox.Cancel)
                return
...
class ObjectList(QtGui.QListWidget):
    ...
    def dragMoveEvent(self, event):
        """ This function is called when the user drags the selected object"."""
    ...
    def startDrag(self, supportedActions):
        """ 
        This function is called when the user clicks on an object to drag it

        From "Rapid GUI programming with Python and Qt, p317-319:
        If you also want this custom window to support drag, and the window is 
        inherited from QWidget, or from some QWidget subclasses without setDragEnabled(), 
        you must do two things to make the widget support drag . One thing is to provide 
        a startDrag() method so that a QDrag object can be created, and the other is to
        ensure that the startDrag() method can be called at the appropriate time.
        The startDrag() method is called automatically by PyQt when needed because we set 
        drag enabled in the initializer.

        The variable supportedActions is not used !
        ...
        # On les ajoute dans le dataStream et ces informations pourront etre recuperees 
        # par la classe mooringWidget
...
class MooringWindow(QtGui.QWidget):
    ...
    def __init__(self, default_folder, screen_width, parent=None):
        ...
        self.mypath = self.mypath+'\Library'
        #Creation toolbar#
        refresh_library = QtGui.QAction(QtGui.QIcon(
        ...
        self.frameLayout2 = QtGui.QVBoxLayout()
